...
function compareWithCutoff(
	used: SemVer,
	current: SemVer,
	allowance: SemVer,
	ignoreSkips: boolean = true,
): boolean {
...
export function findRank(used: SemVer, current: SemVer): number {
	...
	if (
		!compareWithCutoff(
			used,
			current,
			semVerFromString(config.rankCutoff.major),
		)
	) {
		return 0;
	} else if (
		...
		!compareWithCutoff(
			used,
			current,
			semVerFromString(config.rankCutoff.minor),
		)
	) {
		return 1;
	}
