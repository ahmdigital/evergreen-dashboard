...
export function applySort(
	...
	switch (sortSetting.type) {
		...
		case "time":
			rows.sort((a, b) =>
				dayjs(b.lastUpdated).diff(dayjs(a.lastUpdated)),
			);
			break;
		case "internal":
			rows.sort(
				(a, b) => a.internalSubRows.length - b.internalSubRows.length,
			);
			break;
		case "external":
			rows.sort(
				(a, b) => a.externalSubRows.length - b.externalSubRows.length,
			);
			break;
		case "total":
			rows.sort(
				(a, b) =>
					a.internalSubRows.length +
					a.externalSubRows.length -
					(b.internalSubRows.length + b.externalSubRows.length),
			);
			break;
		case "users":
