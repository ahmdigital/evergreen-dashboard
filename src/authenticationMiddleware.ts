...
export async function checkAuthorisation(
	...
	if (
		((process.env.REQUIRE_AUTHENTICATION ?? "").toLowerCase() === "true"
			? true
			: false) == false
	) {
		console.log(
			"REQUIRE_AUTHENTICATION is set to 'false', granting access to user",
		);
		return true;
	}
	...
	try {
		...
		const isMember = await isOrganisationMember(
			process.env.NEXT_PUBLIC_TARGET_ORGANISATION as string,
			username,
		...
		if (!isMember) {
			console.log(
				`User ${username} is not a member of ${process.env.NEXT_PUBLIC_TARGET_ORGANISATION}`,
			);
		...
		console.log(
			`User ${username} is a member of ${process.env.NEXT_PUBLIC_TARGET_ORGANISATION}`,
		);
		return true;
	} catch (error: any) {
		if (error?.status == 401) {
