	...
	return (
		<>
			<Box sx={{ backgroundColor: "#f5f5f5" }}>
				<Tabs
					subRows={{
						internal: props.internal.map((data, i) => (
							<SubRow key={i} data={data} forUsers={false} />
						)),
						external: props.external.map((data, i) => (
							<SubRow key={i} data={data} forUsers={false} />
						)),
						user: props.users.map((data, i) => (
							<SubRow key={i} data={data} forUsers={true} />
						)),
						final: false,
					}}
					tableFunc={(
						elem: ReactNode,
						variant: "dependency" | "user"
					) => {
						return (
							<Box
								sx={{
							...
							>
								<Grid container columnSpacing={2}>
									<Grid item>
										<Typography sx={subRowHeaderTextSX}>
											status&nbsp;
										</Typography>
									</Grid>
									<Grid item xs>
