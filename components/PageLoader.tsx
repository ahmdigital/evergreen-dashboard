...
async function getDataFromAPI(
	api: "loadNew" | "loadLatest" | "forceNew",
	request: "npm" | "PyPI" | "RubyGems"
) {
	...
	while (!JSObject.ok) {
		...
		if (JSObject.status == 401 || JSObject.status == 403) {
			...
			if (error?.message === "login_required") {
				...
				window.location.href = `/signin?error=${encodeURIComponent(
					error?.message
				)}`;
			}
		}
	...
	const result = JSObjectFromJSON(
		getProperty(data! as { npm: any; PyPI: any; RubyGems: any }, request)
	);
...
export function PageLoader(
			...
			case Mode.StandaloneBackend:
				{
					const data = getProperty(
						(cachedData as { npm: any; PyPI: any; RubyGems: any })!,
						request
					);
			...
			case Mode.IntegratedBackend:
				{
					getCurrentVersion(request)
						...
						.then(async () => {
							await getNewVersion(request).then(
								async (result) => {
									setData(result as any);
									setLoading(false);
								}
	...
	if (isLoading) {
		if (mode == Mode.IntegratedBackend) {
			...
			if (
				data != null &&
				(data! as { oldVersion: boolean; data: any }).oldVersion
			) {
				return (
					<Page
						JSObject={
							(data as { oldVersion: boolean; data: any }).data
						}
						finalData={false}
				...
				data != null &&
				(data! as { refreshing: boolean; data: any }).refreshing
			) {
				return (
					<Page
						JSObject={
							(data as { refreshing: boolean; data: any }).data
						}
						finalData={false}
						targetOrganisation={props.targetOrganisation}
					/>
